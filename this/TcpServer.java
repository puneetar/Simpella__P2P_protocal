	...
	public void run(){
		try {
			...
			//	System.out.println("the class of the object is :"+b.getClass());
				...
				if(clientMessage.equalsIgnoreCase(Echoer.SIMPELLA_CONNECT)){
					if(Database.in_conn.size()<3){
						out.writeObject(Echoer.SIMPELLA_OK);
						System.out.println("Accepted the Incomming Connection");
						continue;
					}
					else{
						out.writeObject(Echoer.SIMPELLA_ERROR);
						System.out.println("Rejected Connection due to: Maximum number of connections reached");
						break;
						
					}
				}

				if(clientMessage.equalsIgnoreCase(Echoer.SIMPELLA_THANK) && Database.in_conn.size()<3){
					//out.writeObject(Echoer.SIMPELLA_OK);
